module Blink.View.Controls.ElementSpec (spec) where

import qualified Data.Map.Strict as Map
import Test.Hspec

import Blink.View.Controls.Element
...
import Blink.View.Controls.ElementBehaviour (elementBehaviourSpec)
...
-- plus 'elementId' @eid@, discarding the 'Blink.View.Controls.Element.ElementInteraction'
-- it returns -- the render function every behaviour spec in this module
-- drives.
render :: Ord e => e -> [Attribute (ElementConfig e msg)] -> View e msg ()
...
-- with @e@ and a plain label naming which one it was -- "Blink.View.Controls.Element"
...
both :: View TestElement (TestElement, String) ()
both = do
...
seedBothCtx :: ViewContext TestElement (TestElement, String)
seedBothCtx = emptyViewContext testBounds noInput testTheme noOpTextMeasurer

spec :: Spec
spec = describe "Blink.View.Controls.Element.elementBase" $ do
  ...
  describe "onKeyPressed" $
    it "reacts with the triggering KeyEvent" $ do
      ...
      ctx0 <- snd <$> runView (setFocus ElemA) (emptyViewContext testBounds noInput testTheme noOpTextMeasurer)
      ctx  <- snd <$> runView (render ElemA attrs) (advance (noInput { inputKeyEvents = [KeyEvent KeyReturn [] False] }) ctx0)
