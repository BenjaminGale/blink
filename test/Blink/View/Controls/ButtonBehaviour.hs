...
-- every 'Blink.View.Controls.Button.buttonBase'-based control must satisfy, on
...
-- reflecting its 'Blink.View.Controls.Button.ActivateOnPress' Enter-repeat
-- behaviour, since it genuinely differs from the rest.
module Blink.View.Controls.ButtonBehaviour
...
-- ('Blink.View.Controls.Button.ActivateOnClick'-based) default -- passed by
-- 'Blink.View.Controls.Button.ActivateOnPress'-based controls, i.e.
-- 'Blink.View.Controls.RepeatButton.repeatButton'.
newtype ButtonBehaviourConfig = ButtonBehaviourConfig
  { bbcRepeatsOnHeldEnter :: Bool
    ...
    -- 'Blink.View.Controls.Button.ActivateOnClick'-based control, where a
    -- single logical key press is the whole interaction; 'True' for an
    -- 'Blink.View.Controls.Button.ActivateOnPress'-based one, where holding
    -- Enter is the keyboard equivalent of holding the mouse down -- see
    -- 'Blink.View.Controls.Button.ButtonActivation'.
  }

-- | 'bbcRepeatsOnHeldEnter' 'False' -- what every 'Blink.View.Controls.Button.ActivateOnClick'-based
-- control (every button-family control except 'Blink.View.Controls.RepeatButton.repeatButton') does.
...
buttonBehaviourSpec cfg bounds ctx eid marginPoint insideRect outsidePoint render = do
  ...
  describe "keyboard activation" $ do
    ...
    it repeatDescription $ do
      ...
      (_, afterRepeat) <- runView (render taggedActivated) repeatCtx
