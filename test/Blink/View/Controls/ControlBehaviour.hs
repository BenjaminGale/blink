...
-- 'Blink.View.Controls.Label.label' never auto-claiming or taking focus on a
-- plain click) passes a 'ControlBehaviourConfig' reflecting that, rather
-- than skipping this contract altogether.
module Blink.View.Controls.ControlBehaviour
...
-- special case like 'Blink.View.Controls.Label.label' or 'Blink.View.Controls.ProgressBar.progressBar'.
...
data ControlBehaviourConfig = ControlBehaviourConfig
  ...
  , cbcClickFocuses :: Bool
    ...
    -- with no 'Blink.View.Controls.Label.target').
  }
...
controlBehaviourSpec cfg bounds ctx eid marginPoint insideRect outsidePoint render = do
  ...
  describe "click and keyboard focus" $ do
    ...
    -- 'Blink.View.Controls.Label.label' with no target).
