...
-- every 'Blink.UI.Controls.Button.buttonBase'-based control must satisfy, on
-- top of the raw-event\/focus\/hit-region contract every control already
-- satisfies (see 'Blink.UI.Controls.ControlBehaviour.controlBehaviourSpec').
-- 'Blink.UI.Controls.ButtonSpec' runs this against 'Blink.UI.Controls.Button.button';
-- 'Blink.UI.Controls.Toggle.toggleButton', 'Blink.UI.Controls.Checkbox.checkbox',
-- 'Blink.UI.Controls.RadioButton.radioButton', and
-- 'Blink.UI.Controls.RepeatButton.repeatButton' reuse it too, on top of their
-- own control-specific contract -- the last passing a 'ButtonBehaviourConfig'
-- reflecting its 'Blink.UI.Controls.Button.ActivateOnPress' Enter-repeat
-- behaviour, since it genuinely differs from the rest.
module Blink.UI.Controls.ButtonBehaviour
...
import Blink.UI.Controls.Button (HasButtonConfig, onActivated)
import Blink.UI.Controls.Control (Attribute, HasControlConfig, HasElementConfig, isFocusable)
import Blink.UI.Controls.ControlBehaviour (controlBehaviourSpec, defaultControlBehaviourConfig)
import Blink.UI.Controls.ElementBehaviour (tagged)
import Blink.Geometry (Point, Rectangle)
import Blink.Input (InputState (..), Key (KeyReturn), KeyEvent (..))
import Blink.Interaction (Interaction (..), InteractionResult (..), runInteractions)
import Blink.UI

-- | Every raw\/focus reaction (including 'Blink.UI.Controls.Element.onClicked',
...
-- ('Blink.UI.Controls.Button.ActivateOnClick'-based) default -- passed by
-- 'Blink.UI.Controls.Button.ActivateOnPress'-based controls, i.e.
-- 'Blink.UI.Controls.RepeatButton.repeatButton'.
newtype ButtonBehaviourConfig = ButtonBehaviourConfig
  { bbcRepeatsOnHeldEnter :: Bool
    ...
    -- 'Blink.UI.Controls.Button.ActivateOnClick'-based control, where a
    -- single logical key press is the whole interaction; 'True' for an
    -- 'Blink.UI.Controls.Button.ActivateOnPress'-based one, where holding
    -- Enter is the keyboard equivalent of holding the mouse down -- see
    -- 'Blink.UI.Controls.Button.ButtonActivation'.
  }

-- | 'bbcRepeatsOnHeldEnter' 'False' -- what every 'Blink.UI.Controls.Button.ActivateOnClick'-based
-- control (every button-family control except 'Blink.UI.Controls.RepeatButton.repeatButton') does.
...
-- 'Blink.UI.Controls.Element.onClicked' (mouse-only, per the split between the
-- two).
buttonBehaviourSpec
