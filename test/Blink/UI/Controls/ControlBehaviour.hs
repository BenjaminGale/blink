...
-- | The shared focus\/hit-region contract every 'Blink.UI.Controls.Control.controlBase'-based
-- control must satisfy, on top of the raw-event contract every element
-- already satisfies (see 'Blink.UI.Controls.ElementBehaviour.elementBehaviourSpec').
-- 'Blink.UI.Controls.ControlSpec' runs this against 'Blink.UI.Controls.Control.controlBase'
...
-- 'Blink.UI.Controls.Label.label' never auto-claiming or taking focus on a
-- plain click) passes a 'ControlBehaviourConfig' reflecting that, rather
-- than skipping this contract altogether.
module Blink.UI.Controls.ControlBehaviour
...
import Blink.UI.Controls.Control (Attribute, HasControlConfig, HasElementConfig, isEnabled, isFocusable)
import Blink.UI.Controls.ElementBehaviour (elementBehaviourSpec, tagged)
...
-- special case like 'Blink.UI.Controls.Label.label' or 'Blink.UI.Controls.ProgressBar.progressBar'.
...
data ControlBehaviourConfig = ControlBehaviourConfig
  ...
  , cbcClickFocuses :: Bool
    ...
    -- with no 'Blink.UI.Controls.Label.target').
  }
...
-- 'Blink.UI.Controls.ElementBehaviour.elementBehaviourSpec'.
controlBehaviourSpec
...
controlBehaviourSpec cfg bounds ctx eid marginPoint insideRect outsidePoint render = do
  ...
  -- contract about the same raw facts 'Blink.UI.Controls.ElementSpec' checks, not
  ...
  describe "click and keyboard focus" $ do
    ...
    -- 'Blink.UI.Controls.Label.label' with no target).
