...
module Blink.UI.Controls.DividerSpec (spec) where

import qualified Data.Map.Strict as Map
import Test.Hspec

import Blink.UI.Controls.Control (Attribute, elementId)
import Blink.UI.Controls.ControlBehaviour (ControlBehaviourConfig (..), controlBehaviourSpec)
import Blink.UI.Controls.Divider (DividerConfig, divider, orientation, thickness)
import Blink.UI.Controls.ElementBehaviour (tagged)
import Blink.UI.Controls.FixedFocusBehaviour (fixedNotFocusableSpec)
import Blink.Geometry (Alignment (Center), Orientation (..), Point (..), Rectangle (..), insetRect, noBorder, uniform)
import Blink.Input (InputState (..))
import Blink.Interaction (Interaction (..), InteractionResult (..), runInteractions)
import Blink.UI.Layout.Constraints (align, exactly, width)
...
spec :: Spec
spec = describe "Blink.UI.Controls.Divider" $ do
