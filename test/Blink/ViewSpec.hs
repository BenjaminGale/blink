...
module Blink.ViewSpec (spec) where
...
spec = describe "Blink.View" $ do
  ...
  describe "focus change (Focus / ClearFocus)" $ do
    it "a focus request sets the new focus and reports it to the winner and the loser" $ do
      ...
      (loserGained, _)   <- runView (hasGainedFocus ElemA) ctx1
      (winnerLost, _)    <- runView (hasLostFocus ElemB) ctx1
    ...
    it "the recorded change stays visible for exactly one more frame, then is cleared" $ do
      ...
      (gainedNextFrame, _)  <- runView (hasGainedFocus ElemB) ctx2
      (lostNextFrame, _)    <- runView (hasLostFocus ElemA) ctx2
      (gainedFrameAfter, _) <- runView (hasGainedFocus ElemB) ctx3
      (lostFrameAfter, _)   <- runView (hasLostFocus ElemA) ctx3
