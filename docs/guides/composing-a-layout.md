...
`top`/`left`/`centre` each take a plain `UI e msg ()` action, not an
...
bare `UI e msg ()` too, for the same reason: it already knows its own
...
return `UI e msg ()` directly for exactly this reason — see the next
section.

Since a view must return an `Element` (not a bare `UI` action),
...
resulting `Element` back to the bare `UI e msg ()` a border panel expects
with `runElement`:

```haskell
sidebar :: AppState -> UI ScreenId Msg ()
...
* **Children take more space than the box.** The overflow is clipped, not
  ...
  built around `Blink.UI`'s scroll primitives (`elmScrollStates`, see
  "Blink.UI" for the accessors) rather than relying on `vBox` to do it for
  you.

## Converting between `Element` and a bare `UI` action

The pattern above — `runElement` to go from `Element` to a bare `UI`
...
* **`Element` → `UI e msg ()`**: `runElement`. Use this when something
  ...
  as above, or any hand-written `UI` code that just needs to run a widget
  without placing it in another sized slot.
* **`UI e msg ()` → `Element`**: `elementWithLayout layout action`. Use
  this when a plain `UI` action (hand-written primitives, or the result of
