...
Instead, it queues a description of a change — an **effect** — for
something else to apply once this frame's tree walk is finished. There
are two flavours of this in `Blink.View`, aimed at two different owners:
...
`update`. See `Blink.View`'s Haddocks for `emitUi`'s exact signature.
