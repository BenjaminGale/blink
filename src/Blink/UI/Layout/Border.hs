...
import Blink.UI.Layout.Box (children, hBox, vBox)
import Blink.UI.Layout.Constraints (Layout (..), exactly, fill)
import Blink.UI (UI)
...
--
-- No spacing or margin is applied. Clipping follows 'Blink.UI.Layout.Box.vBox' and
-- 'Blink.UI.Layout.Box.hBox': the top, middle, and bottom rows are clipped as a
