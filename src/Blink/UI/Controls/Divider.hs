...
-- "Blink.UI.Controls": no value, and -- unlike every other control -- no id
-- required.
module Blink.UI.Controls.Divider
  ( DividerConfig (..)
...
import Blink.UI.Layout.Constraints (HasLayoutConfig (..), Layout (..), fill, fitContent)
...
-- either axis afterwards with 'Blink.UI.Layout.Constraints.width'\/
-- 'Blink.UI.Layout.Constraints.height' as usual.
...
-- 'Blink.UI.Layout.Box.vBox'; 'Vertical' draws one stretching top-to-bottom,
-- for separating things side by side in an 'Blink.UI.Layout.Box.hBox'.
-- Resets the default 'Layout' the new axis implies -- apply this before any
-- 'Blink.UI.Layout.Constraints.width'\/'Blink.UI.Layout.Constraints.height'
...
-- running it along. Defaults to 1. Has no effect if 'Blink.UI.Layout.Constraints.width'\/
-- 'Blink.UI.Layout.Constraints.height' overrides that axis to something other
-- than 'Blink.UI.Layout.Constraints.fitContent'.
...
-- fixed behaviour, not a default, the same way 'Blink.UI.Controls.ProgressBar.progressBar'
...
-- override with 'Blink.UI.Layout.Constraints.width'\/'Blink.UI.Layout.Constraints.height'\/
-- 'Blink.UI.Layout.Constraints.align'; when placed in an 'Blink.UI.Layout.Box.hBox'\/
-- 'Blink.UI.Layout.Box.vBox' next to a taller\/wider sibling,
-- 'Blink.UI.Layout.Constraints.align' picks where within that extra space the
-- line sits.
...
divider attrs = Element
  ...
  where
    ...
    -- see 'Blink.UI.Layout.Constraints.resolveLength'.
