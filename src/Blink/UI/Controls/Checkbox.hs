...
-- it needs.
module Blink.UI.Controls.Checkbox
  ( checkbox
  , checkboxStyleKey
  ) where
...
-- selected (see 'Blink.UI.Controls.Toggle.isSelected'), beside a caption set via 'Blink.UI.Controls.Label.text', toggled
-- together as one control -- clicking either the box or the caption
-- activates it, the same as 'Blink.UI.Controls.Toggle.toggleButton'. Flips
-- every time it's activated; see 'Blink.UI.Controls.Toggle.onSelectedChanged' for reacting to it.
-- Defaults to sizing itself to its own glyph-plus-caption content on both
-- axes, the same as 'Blink.UI.Controls.Label.label'; override with
