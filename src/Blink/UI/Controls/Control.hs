...
-- "Blink.UI.Controls" is built from -- see "Blink.UI.Controls.Element" for the
...
module Blink.UI.Controls.Control
  ( -- * Re-exported from Blink.UI.Controls.Element
    Attribute (..)
  , resolve
  , EventHandler
  , KeyEventHandler
...
import Blink.UI.Controls.Element
...
import Blink.UI.Layout.Constraints (MeasureCtx (..), shrink)
...
data ControlConfig e msg = ControlConfig
  ...
  , ccActiveStates :: Set VisualState
    ...
    -- 'Blink.UI.Controls.Toggle.toggleBase' setting a checked\/unchecked
...
controlBase cc = disableWhen (not (ccIsEnabled cc)) $
  ...
  where
    ...
    renderTracked eid = do
      ...
      -- 'Blink.UI.Controls.Label.target' pointed at it isn't stopped that
...
-- itself when clicked -- e.g. 'Blink.UI.Controls.Label.label' redirecting
-- onto its 'Blink.UI.Controls.Label.target'.
