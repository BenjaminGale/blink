...
-- "Blink.UI.Controls" is built on -- see "Blink.UI.Controls.Control" for the one
-- above it. Re-exports "Blink.Attribute"'s 'Attribute'\/'resolve' mechanism, which
-- every layer here (and "Blink.UI.Layout.Box") is configured through.
--
-- = The attribute mechanism
--
-- Each layer's own config type gets a one-method typeclass
-- (@HasElementConfig@, 'Blink.UI.Controls.Control.HasControlConfig') so an
...
module Blink.UI.Controls.Element
  ( -- * Attributes
    Attribute (..)
  , resolve
...
-- 'eiClicked' -- and, transitively, into "Blink.UI.Controls.Control"'s
...
data MouseActivation
  ...
  | CaptureActivated
    ...
    -- 'Blink.UI.Controls.Slider.slider'), the value has already changed by
...
-- 'Blink.UI.Controls.Button.onActivated' for the event that also fires on
...
-- | Builds a reaction (an 'EventHandler'\/'Blink.UI.Controls.Toggle.onSelectedChanged'-shaped
...
-- Shared with "Blink.UI.Controls.Control", whose own auto-claim logic needs
-- the same check.
...
-- 'Blink.UI.Controls.Control.controlBase') can read the same flags back
