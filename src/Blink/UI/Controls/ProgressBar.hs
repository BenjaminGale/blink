...
-- label, so it has no 'Blink.UI.Controls.Label.LabelledConfig' either.
module Blink.UI.Controls.ProgressBar
  ( ProgressBarConfig (..)
  , ProgressValue (..)
...
import Blink.UI.Layout.Constraints (HasLayoutConfig (..), Layout (..), fill)
...
-- Takes no id by default -- pass 'Blink.UI.Controls.Control.elementId' to
...
-- control did before controls reported their own 'Blink.UI.Layout.Layout'.
