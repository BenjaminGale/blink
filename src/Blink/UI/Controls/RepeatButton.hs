...
-- | A button that keeps firing 'Blink.UI.Controls.Button.onActivated' at a
...
-- increment\/decrement. Built on 'Blink.UI.Controls.Button.buttonBase' the
-- same way 'Blink.UI.Controls.Button.button' is, configured with
-- 'Blink.UI.Controls.Button.ActivateOnPress' -- see 'repeatButton'.
--
-- @
...
-- like 'Blink.UI.Controls.Slider.slider' owning its value, the caller owns
...
module Blink.UI.Controls.RepeatButton
  ( RepeatButtonConfig (..)
...
import Blink.UI.Controls.Button
...
import Blink.UI.Layout.Constraints (HasLayoutConfig (..))
import Blink.UI
import Blink.UI.Element (Element (..))

-- | Every capability 'repeatButton' resolves: the wrapped 'ButtonConfig'
-- (styling, caption, layout, and 'Blink.UI.Controls.Button.onActivated'
...
-- while held -- anything that drops 'Blink.UI.Controls.Element.eiHeld').
...
-- | A button that fires 'Blink.UI.Controls.Button.onActivated' once
-- immediately on press (not on release, unlike
-- 'Blink.UI.Controls.Button.button' -- holding is the whole point, so waiting
...
-- 'Blink.UI.Controls.Button.ActivateOnPress', which this forces -- see
-- 'Blink.UI.Controls.Button.ButtonActivation'), but at whatever cadence the
...
repeatButton eid attrs = Element
  ...
  where
    ...
    -- idiom as 'Blink.UI.Controls.RadioButton.radioButton' forcing 'tgcNext').
