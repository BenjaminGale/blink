...
-- 'Blink.Controls.RadioButton.radioButton' directly (e.g.
...
-- 'Blink.Controls.RadioButton.radioButton' never reports its own
...
-- constructor -- 'Blink.Controls.ToggleButton.toggleButton' or
-- 'Blink.Controls.RadioButton.radioButton' -- to build one item from),
...
-- 'Blink.Controls.ToggleButton.toggleBase', which only fires when
...
-- back to unselected on its own (a 'Blink.Controls.ToggleButton.toggleButton');
-- the group only acts on that when 'allowDeselect' is set, otherwise it's a
-- no-op. A 'Blink.Controls.RadioButton.radioButton' never reports
-- 'False' from being clicked again at all, so it behaves as a plain,
-- never-deselectable radio group regardless of 'allowDeselect'.
--
-- The group itself is a genuine 'Blink.Controls.Control.control' --
-- 'Blink.Controls.Control.style'-able, disabled as a whole via
-- 'Blink.Controls.Control.isEnabled' (which disables every item too,
...
-- never a keyboard focus target ('Blink.Controls.Control.NotFocusable'
