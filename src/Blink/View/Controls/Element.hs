...
-- (@HasElementConfig@, 'Blink.View.Controls.Control.HasControlConfig') so an
...
module Blink.View.Controls.Element
  ( -- * Attributes
    Attribute (..)
  , resolve
...
-- 'eiClicked' -- and, transitively, into "Blink.View.Controls.Control"'s
...
-- Shared with "Blink.View.Controls.Control", whose own auto-claim logic needs
-- the same check.
...
-- 'Blink.View.Controls.Control.controlBase') can read the same flags back
