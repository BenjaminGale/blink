...
By default, a 'Blink.View.View' action fills the full space it is given by its parent.
...
This module is a barrel over "Blink.View.Layout.Constraints" (single-child sizing and
positioning), "Blink.View.Layout.Box" (arranging several children along an
axis), and "Blink.View.Layout.Border" (dividing space into named regions) --
...
module Blink.View.Layout
  ...
  , preferredSize
    ...
    --
    --   Both are special cases: they draw on 'Blink.View.getBounds' \/ 'Blink.View.withBounds', the
    ...
    --   'Blink.View.getBounds' from inside the 'Blink.View.View' monad, or the application state,
    ...
    --   this because a plain 'Blink.View.View' action and a plain function argument
    --   already do the job:
    --
    --   @
    --   contextual :: AppState -> View Element msg ()
    --   contextual state = do
    --     bounds <- getBounds
    --     runElement $
    ...
    --   'Blink.View.measureText' rather than writing one down:
    --
    --   @
    --   -- Sizes a label column exactly as wide as its own caption,
    --   -- instead of guessing a fixed width.
    --   labelledRow :: Text -> View Element msg () -> View Element msg ()
