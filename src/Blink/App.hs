...
import Blink.UI (UI, UIContext (..), UIState (..), emptyUIState, runUI, drawCalls, pendingCommands, focusedElement, focusedRendered)
...
runApp backend app = do
  state <- startUp app
  loop backend app state Nothing

loop :: Backend -> App e s c -> s -> Maybe e -> IO ()
loop backend app state prevFocus = do
  ...
  if close
    then return ()
      ...
      let winRect = Rectangle 0 0 (sizeWidth size) (sizeHeight size)
          ...
          freshSt = emptyUIState { focusedElement = prevFocus }
      ...
      loop backend app state' nextFocus
